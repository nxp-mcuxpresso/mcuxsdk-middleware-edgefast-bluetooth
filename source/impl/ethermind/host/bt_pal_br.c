...
static sys_slist_t discovery_cbs;
...
static void appl_hci_esco_audio_config(uint8_t coding)
{
     ...
     * Setup Config
     */
    sco_audio_config.num_ch = 0x01;     /* No. of channels: Mono */
    sco_audio_config.sample_len = 16;   /* Each PCM sample size */
    sco_audio_config.sampl_freq = (LMP_VOICE_AIR_CODING_TRANSPARENT == coding)? 16000: 8000; /* Sampling frequency */
    sco_audio_config.data_size  = 512; /* Record data length */
...
void bt_hci_synchronous_conn_complete(struct net_buf *buf)
{
    ...
    appl_hci_esco_audio_config(evt->air_mode);
