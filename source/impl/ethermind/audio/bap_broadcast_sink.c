...
#include "../host/bt_pal_iso_internal.h"

#include "audio_internal.h"
#include "bap_iso.h"
#include "bap_endpoint.h"

#define BT_DBG_ENABLED IS_ENABLED(CONFIG_BT_DEBUG_BAP_BROADCAST_SINK)
#define LOG_MODULE_NAME bt_bap_broadcast_sink
...
static enum bt_bap_ep_state broadcast_sink_get_state(struct bt_bap_broadcast_sink *sink)
{
	...
	SYS_SLIST_FOR_EACH_CONTAINER(&sink->streams, stream, _node) {
		if (stream->ep != NULL) {
			state = MAX(state, (enum bt_bap_ep_state)stream->ep->status.state);
		}
	}
...
static bool merge_bis_and_subgroup_data_cb(struct bt_data *data, void *user_data)
{
	...
	err = bt_audio_codec_cfg_set_val(codec_cfg, (enum bt_audio_codec_cfg_type)data->type, data->data, data->data_len);
	if (err < 0) {
			 ...
			 const struct bt_iso_biginfo *biginfo)
{
	...
	sink->codec_qos.framing = (enum bt_audio_codec_qos_framing)biginfo->framing;
...
#else
int bt_bap_broadcast_sink_init(void)
#endif
{
