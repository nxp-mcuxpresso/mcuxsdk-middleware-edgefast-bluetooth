...
#if BT_BAP_BIS_SYNC_NO_PREF < UINT32_MAX /* EDGEFAST: Useless condition */
...
#if 0 /* EDGEFAST: Useless condition */
...
{
    if ((shell_status_t)kStatus_Success != SHELL_RegisterCommand(shell, &g_shellCommandbap_scan_delegator))
    {
        shell_print(shell, "Shell register command %s failed!", g_shellCommandbap_scan_delegator.pcCommand);
    }
}
